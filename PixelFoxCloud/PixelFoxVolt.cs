...
namespace PixelFoxCloud
{
    public class FoxPvPower : PixelModule
    {
        ...
        protected override void Update(Stopwatch sw) { Visible = FoxService.Instance.ShouldShow; }

        public override void Draw(Stopwatch sw)
        {
            Screen.SetImage(_logo.Advance(), 0);
            if (!FoxService.Instance.HasData) { Screen.SetText("---", Color.DarkRed, 9); return; }
            float p = FoxService.Instance.PvPower;
    ...
    public class FoxToday : PixelModule
    {
        ...
        public override void Draw(Stopwatch sw)
        {
            ...
            if (!FoxService.Instance.HasData) { Screen.SetText("---", Color.DarkRed, 9); return; }
            int pos = Screen.SetText(FoxService.Instance.TodayYield.ToString("#.#"), Color.DarkGoldenrod, 9, 1);
